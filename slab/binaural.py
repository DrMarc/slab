...
class Binaural(Sound):
	...
	def azimuth_to_itd(azimuth, frequency=2000, head_radius=8.75):
		...
		itd_2000 = (head_radius / speed_of_sound) * (azimuth_radians + numpy.sin(azimuth_radians)) # Woodworth
		itd_500 = (3 * head_radius / speed_of_sound) * numpy.sin(azimuth_radians)
		itd = numpy.interp(frequency, [500, 2000], [itd_500, itd_2000], left=itd_500, right=itd_2000)
		return itd
	...
	def at_azimuth(self, azimuth=0):
		...
		'''
		centroid = self.spectral_centroid().mean()
	...
	def _make_level_spectrum_filter(hrtf=None):
		...
		idx = numpy.where((hrtf.sources[:,1] == 0) & ((hrtf.sources[:,0] <= 90) | (hrtf.sources[:,0] >= 270)))[0]
		...
		azi[azi>=270] = azi[azi>=270]-360 # 270<azi<360 -> azi-360 to get negative angles the left
