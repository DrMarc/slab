...
def Key():
    '''
    Wrapper for curses module to simplify getting a single keypress from the
    terminal. Use like this:
    with slab.Key() as key:
            response = key.getch()
    '''
    if not have_curses:
        raise ImportError('You need curses to use the keypress class'
                          ' (pip install curses (or windows-curses))')
...
class LoadSaveJson:
    ...
    def save_json(self, file_name=None):
        ...
        """
        # self_copy = copy.deepcopy(self) use if reading the json file fails
...
class Trialsequence(collections.abc.Iterator, LoadSaveJson):
    # TODO: correct string conditions!
    ...
    conditions: an integer, list, or flat array specifying condition indices,
            ...
            If conditions is a string, then it is treated as the name of a
            previously saved trial sequence object, which is then loaded.
    n_reps: number of repeats for all conditions
    trials: a list of condition indices, i.e. the trial sequence. Typically,
            this list is left empty and generated by the class based on the
            other parameters.
    kind: The kind of sequence randomization used to generate the trial
          sequence. 'non-repeating' (conditions are repeated in random order
          n_reps times without direct repetition - default if n_conds > 2), or
          'random_permutation' (conditions are permuted randomly without
          control over transition probabilities - default if n_conds <= 2).
...
"""
    ...
    def __init__(self, conditions=2, n_reps=1, trials=[], kind=None, name=''):
        ...
        if not trials:
            ...
            if kind == 'non_repeating':
                trials = Trialsequence._create_simple_sequence(
                    len(self.conditions), self.n_reps)
            elif kind == 'random_permutation':
                trials = Trialsequence._create_random_permutation(
                    len(self.conditions), self.n_reps)
            else:
    ...
    def __str__(self):
        return f'Trialsequence, trials {self.n_trials}, \
            remaining {self.n_remaining}, current condition {self.this_trial}'
    ...
    def __next__(self):
        ...
        if self.finished:
            raise StopIteration
        self.this_trial = self.conditions[self.trials[self.this_n]]
        return self.this_trial

    @staticmethod
    def _create_simple_sequence(n_conditions, n_reps):
        '''Create a sequence of n_conditions x n_reps trials, where each
        repetitions contains all conditions in random order, and no condition
        is directly repeated across repetitions.'''
    ...
    def _create_random_permutation(n_conditions, n_reps):
        '''Create sequence of n_conditions x n_reps trials in random order.'''
        return list(numpy.random.permutation(
            numpy.tile(list(range(n_conditions)), n_reps)))
    ...
    def condition_probabilities(self):
        """Return list of the frequencies of conditions in the order
        listed in self.conditions"""
        probs = []
    ...
    def mmn_sequence(n_trials, deviant_freq=0.12):
        ...
        trials = []  # make trial sequence by putting possibilities together
        for i in idx:
            trials.extend(partials[i])
        trials = trials[:n_trials]  # cut list to requested number of trials
...
class Staircase(collections.abc.Iterator):
    ...
    >>> stairs = Staircase(start_val=50, n_reversals=10, step_type='lin',
                    step_sizes=[4,2], min_val=10, max_val=60, n_up=1,
                    n_down=1, n_trials=10)
    >>> print(stairs)
    ...
    def __init__(self, start_val, n_reversals=None, step_sizes=4, n_trials=0,
                 n_up=1, n_down=2, step_type='lin', min_val=None,
                 max_val=None, name=''):
        """
        :Parameters:
                name:
                ...
                n_reversals:
                        ...
                        length of this list, PsychoPy will automatically
                        increasE the minimum number of reversals and emit
                        a warning.
                step_sizes:
                        The size of steps as a single value or a list
                        (or array). For a single value the step size is fixed.
                        For an array or list the step size will progress to
                        the next entry at each reversal.
                n_trials:
                        The minimum number of trials to be conducted. If the
                        staircase has not reached the required number of
                        reversals then it will continue.
                n_up:
                ...
                min_val: *None*, or a number
                        The smallest legal value for the staircase, which can
                        be used to prevent it reaching impossible contrast
                        values, for instance.
        ...
        if n_reversals is None:
            ...
            print(f'Increasing number of minimum required reversals to the \
                number of step sizes, {len(self.step_sizes)}')
            self.n_reversals = len(self.step_sizes)
        else:
        ...
        # psychometric functions, auto set when finished
        self.pf_intensities = None
        self.pf_percent_correct = None
        self.pf_responses_per_intensity = None
    ...
    def __next__(self):
        ...
        else:  # tally responses to create a psychometric function
    ...
    def __str__(self):
        return f'Staircase {self.n_up}up-{self.n_down}down, \
            trial {self.this_trial_n}, {len(self.reversal_intensities)} \
            reversals of {self.n_reversals}'
    ...
    def add_response(self, result, intensity=None):
        ...
        if self._next_intensity <= self.min_val:
            result = False  # always record False if at min_val
        else:
            result = bool(result)
    ...
    def calculatenext_intensity(self):
        """Based on current intensity, counter of correct
        responses, and current direction."""
        ...
        if (len(self.reversal_intensities) >= self.n_reversals
                and len(self.intensities) >= self.n_trials):
    ...
    def threshold(self, n=6):
        ...
        if self.finished:
            ...
            if self.step_type == 'lin':
                ...
                return numpy.exp(numpy.mean(
                    numpy.log(self.reversal_intensities[-n:])))

    def print_trial_info(self):
        print(
            f'''trial # {self.this_trial_n}: intensity
            {round(self.intensities[-1],2) if self.intensities[-1] else
            round(self._next_intensity,2)}, going {self.current_direction},
            response {self.data[-1] if self.data else None}''')
    ...
    def plot(self):
        '''Plot the staircase. If called after each trial,
        one plot is created and updated.'''
        if not have_pyplot:
            raise ImportError('Plotting requires matplotlib!')
        ...
        # _, (ax1, ax2) = \  # prepare a second panel for the pf plot
        # plt.subplots(1, 2, sharey='row', gridspec_kw={'width_ratios': [
        #    2, 1], 'wspace': 0.1}, num='stairs')
        # ax2.plot(self.pf_percent_correct, self.pf_intensities)
        # point_sizes = self.pf_responses_per_intensity * 5 # 5 pixels x trial
        # ax2.scatter(self.pf_percent_correct,
        #            self.pf_intensities, s=point_sizes)
    ...
    def present_afc_trial(self, target, distractors,
                          key_codes=list(range(49, 58)), isi=0.25):
        '''
        ...
        if isinstance(distractors, list):  # sound object and list of sounds
            stims = [target].extend(distractors)
        else:  # assuming two sound objects
...
class Resultsfile():
    '''
    A class for simplifying the typical use cases of results files,
    including generating the name, creating the folders, and writing to the
    file after each trial. Examples:
    >>> Resultsfile.results_folder = 'MyResults'
    >>> file = Resultsfile(subject='MS')
    >>> print(file.name)
    '''
    name = property(fget=lambda self: str(self.path.name),
                    doc='The name of the results file.')

    def __init__(self, subject='test'):
        self.subject = subject
        self.path = Path(results_folder / Path(subject) / Path(
            subject + datetime.datetime.now().strftime(
                "_%Y-%m-%d-%H-%M-%S") + '.txt'))
    ...
    def write(self, data, tag=None):
        '''
        Safely write data (must be lines of text or convertable to lines of
        text) to the file. The file is opened just before writing and closed
        immediately after to avoid data loss. Call this method at the end of
        each trial to save the response and trial state. A tag (any string or
        'time') can be prepended. If tag='time', the current time is prepended.
        '''
...
class Precomputed(list):
    '''
    Class for randomly playing pre-computed sound stimuli without direct
    repetition. 'sounds' can be a list of Sound objects, a function, or an
    iterable. This class simplifies generation and presentation of pre-computed
    sound stimuli and is typically used when stimulus generation takes too long
    to happen in each trial. In this case, a list of stimuli is precomputed and
    a random stimulus from the list is presented in each trial, ideally without
    direct repetition. The class allows easy generation of such stimulus lists
    (type 'slab.Precomputed') and keeps track of the previously presented
    stimulus. The list has a play method which automatically selects an element
    other than the previous one for playing, and can be used like an slab.Sound
    object. 'sounds': [list|callable|iterator], the stimulus objects (must have
    a play method)
    'n': [int, *10*], only used if list is a callable, calls it n times
        to make the stimuli
    Examples:
    ...
    >>> stims = slab.Precomputed(  # using a generator
    >>>     (slab.Sound.vowel(vowel=v) for v in ['a','e','i']) )
    ...
    def __init__(self, sounds, n=10):
        ...
        # a function to generate sound objects was passed, call it n times
        elif callable(sounds):
            list.__init__(self, [])
        ...
        elif hasattr(sounds, '__iter__'):  # iterate through object if possible
            for sound in sounds:
                list.append(self, sound)
        else:
            raise TypeError('Unknown type for list argument.')
        self.previous = None  # index of the previously played sound
...
if __name__ == '__main__':
    ...
    # starcaise with reduced step size every two reversals
    stairs = Staircase(start_val=50, n_reversals=10, step_type='lin',
                       step_sizes=[8, 4, 4, 2, 2, 1],
