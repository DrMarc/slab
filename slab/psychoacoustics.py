...
class Trialsequence(collections.abc.Iterator, LoadSaveMixin, TrialPresentationOptionsMixin):
    ...
    Arguments:
        ...
        trials (None | list | numpy.ndarray): The sequence of trials in the order in which they are appearing in
            sequence. Defaults to None since the trials are usually generated by the class based on the other
            parameters. wHowever, it is possible to pass a list or one dimensional array. In that case the parameters
