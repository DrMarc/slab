...
class Trialsequence(collections.abc.Iterator, LoadSave_mixin, TrialPresentationOptions_mixin):
    ...
    Parameters:
        ...
        kind: The kind of sequence randomization used to generate the trial sequence. `non_repeating` (conditions are
            repeated in randome order `n_reps` times without direct repetition, default if n_conditions > 2),
            `random_permutation` (conditions are permuted randomly without control over transition probabilities,
            (default if `n_conditions` <= 2), or `infinite` (`non_repeating` if n_conditions <= 2 or `random_permutation` trial
