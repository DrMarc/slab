...
class HRTF:
    ...
    def write_sofa(self, filename):
        ...
        if self.datatype == 'FIR':
            ...
            sofa.SOFAConventions, sofa.SOFAConventionsVersion = 'SimpleFreeFieldHRTF', '1.0'
        ...
        sofa.Conventions, sofa.Version = 'SOFA', '2.1'
        ...
        if azimuth is not None:
            if isinstance(azimuth, (tuple, list)):
                ...
                for az in range(int(min(azimuth)), int(max(azimuth)) + 1):
                    out.extend(self.cone_sources(cone=az, mode="cone", full_cone=True, tolerance=tolerance))
                az_idx = numpy.unique(out)
            else:
                az_idx = self.cone_sources(cone=float(azimuth), mode="cone", full_cone=True, tolerance=tolerance)
        # elevation cross-section
        if elevation is not None:
            if isinstance(elevation, (tuple, list)):
                out = []
                for el in range(int(min(elevation)), int(max(elevation)) + 1):
                    out.extend(self.cone_sources(cone=el, mode="elevation", full_cone=True, tolerance=tolerance))
                ele_idx = numpy.unique(out)
            else:
                ele_idx = self.cone_sources(cone=float(elevation), mode="elevation", full_cone=True, tolerance=tolerance)
        # combine
